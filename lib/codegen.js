...
function jsonPascalCaseSchemas(jsonYaml) {
    const jsonFixedSchemaNames = {
      ...jsonYaml,
      components: {
        ...jsonYaml.components,
        schemas: _.mapKeys(jsonYaml.components.schemas, (v, k) => {
          return _.upperFirst(setPascalCase(k));
        })
      },
    };

    // const jsonFixedSchemaNames = mapKeysDeep(jsonYaml, (value, key, parentValue, context) => {
    //   return key;
    // });

    const jsonFixedRefs = mapValuesDeep(
      jsonFixedSchemaNames,
      (value, key, parentValue, context) => {
        if (key === '$ref') {
          const parts = value.split('/');
            const schemaName = parts.pop();
            const schemaNameFixed = _.upperFirst(setPascalCase(schemaName));
            parts.push(schemaNameFixed);
            return parts.join('/');
        }
 ...
 */
function jsonYamlToData(jsonYamlRaw) {
  const jsonYaml = jsonPascalCaseSchemas(jsonYamlRaw);
  ...
  swaggerInfo.email = jsonYaml.info.contact ? jsonYaml.info.contact.email : "";
  swaggerInfo.servers = [];
  ...
  _.forEach(jsonYaml.paths, (path, route) => {
    ...
    _.forEach(path, (method, method_name) => {
      ...
      const defaultResponseClassName = `${_.upperFirst(methodName)}Response`;
      ...
      if (typeof responses !== "undefined") {
        const { projectStructureData, responsesObjectList, temporaryDatas } =
          processingResponseData({
            jsonYaml,
            responses,
            projectStructure: projectStructure[domainName],
            dtoObjectList,
            route,
      ...
      walker.on("file", async (root, stats, next) => {
        try {
          ...
          //옵션으로 선택한 데이터 베이스 생성
            ...
            next();
          } else if (stats.name === ".npmignore") {
            fs.renameSync(
              target_dir + `/${stats.name}`,
