...
function processingResponseData({
  ...
  dtoObjectList,
  route,
  defaultResponseClassName,
}) {
  const responseSuccessfull = responses["200"] || responses["201"] || responses["202"];

  if (!responseSuccessfull) {
    throw Error(`${route} => no successful code response`);
  }

  let className = defaultResponseClassName;

  /**
   * @description
   *  x-codegen-request-body-name(Response 이름)을 설정하지 않으면 에러
   */
  if (typeof responseSuccessfull["x-codegen-request-body-name"] === "undefined") {
    console.warn(
      `${route} => path response x-codegen-request-body-name undefined. Using ${defaultResponseClassName} instead`
    );
  } else {
    className = setPascalCase(
      responseSuccessfull["x-codegen-request-body-name"]
    );
  }

  if (dtoObjectList[className]) {
    throw Error(`${route} => dto ${className} already defined`);
  }
  ...
  if (typeof responseSuccessfull.content === "undefined") {
   ...
   */
  if (typeof responseSuccessfull.content !== "undefined") {
    const requestResponseType = Object.keys(responseSuccessfull.content);
    const requestResponseContent =
      responseSuccessfull.content[requestResponseType[0]].schema;
  ...
  projectStructure.importRequestDto.push({
  ...
  return {
    projectStructureData: projectStructure.importRequestDto,
